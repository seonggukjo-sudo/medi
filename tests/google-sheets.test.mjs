...
test("Google Sheets sync uses the existing validated upload pipeline", async () => {
  ...
  assert.match(route, /gviz\/tq/);
  assert.match(route, /fetchPublicTabs/);
  assert.match(route, /sourceMode/);
  assert.match(route, /public_link/);
  assert.match(route, /parseCsvTable/);
  assert.match(route, /hasKoreanHelperRows/);
  assert.match(route, /importableRows/);
  assert.match(route, /rows\.slice\(4\)/);
  assert.match(route, /온라인광고성과!A:Z/);
  assert.match(route, /고정광고비!A:Z/);
  assert.match(route, /CRM성과!A:Z/);
  assert.match(route, /normalizedAdRows/);
  assert.match(route, /Google Sheets sync failed/);
