...
test("저장하지 않은 설정을 이탈 전에 경고하고 마지막 저장본으로 복원한다", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  assert.match(page, /settingsBackupRef/);
  assert.match(page, /!isDataDirty && !isSettingsDirty/);
  assert.match(page, /\[isDataDirty, isSettingsDirty\]/);
  assert.match(page, /isDataDirty \|\| isSettingsDirty/);
  assert.match(page, /discardSettingsChanges/);
  assert.match(page, /마지막으로 저장된 설정으로 되돌렸습니다/);
